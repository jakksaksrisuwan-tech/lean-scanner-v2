...
export function fuseScans(frames, opts = {}) {
  ...
  // final cosmetic pass: whiten crease shading or binarize. ocrSource
  // stays the raw fusion (measured: every pixel intervention before
  // Apple Vision is net-negative for OCR). A deskew+baseline-flatten
  // stage was tried here and reverted — the user's captures are shot
  // straight-on (2-5px curl, <1 deg tilt) so it only added risk;
  // python reference lives in research/baseline_flatten.py.
